	...
	validate(e) {
		...
		if(elFields[fieldName].email) {
			if(!this.validateEmail(fieldValue)) {
				this.errors[fieldName].push('Wrong email address');
			}
		}

		if(fieldValue.length < elFields[fieldName].minlength || fieldValue.length > elFields[fieldName].maxlength) {
			this.errors[fieldName].push(`Field must have minimum ${elFields[fieldName].minlength} and maximum ${elFields[fieldName].maxlength} characters`);
		}
